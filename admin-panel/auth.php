...
function hasPermission($section) {
    ...
    if ($perms === 'all' || ($_SESSION['admin_role'] ?? '') === 'superadmin') {
        return true;
    }
    if (is_array($perms)) {
...
function requirePermission($section) {
    ...
    if (!hasPermission($section)) {
        ...
        <div class="max-w-2xl mx-auto py-16 text-center space-y-4 bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl">
            ...
            <p class="text-xs text-slate-400 leading-relaxed">Your staff / salesman account does not have permission to access <strong>' . htmlspecialchars($section) . '</strong>.<br>Please contact the Super Administrator if you need access to this feature.</p>
